        ...
        <header className='sticky top-0 h-15 z-999'>
            <nav className='h-full sm:px-8 px-3 bg-gray-100'>
                ...
                <div className="mx-auto max-w-[1560px] flex justify-between h-full  items-center">
                    ...
                    <ul className='list-none pl-0 md:flex gap-7 nav-links hidden'>
                        <li><Link className= {isActive("/") ? "nav-link active" : "nav-link"} href="/">Home</Link></li>
                        <li><Link className= {isActive("/about") ? "nav-link active" : "nav-link"} href="/about">About</Link></li>
                        <li><Link className= {isActive("/contact") ? "nav-link active" : "nav-link"} href="/contact">Contact</Link></li>
                        <li><Link className= {isActive("/policy") ? "nav-link active" : "nav-link"} href="/policy">Policy</Link></li>
                    ...
                    <div className={`mobile-menu w-[300px] shadow bg-white md:hidden fixed top-15 z-[900] right-0 min-h-screen transition-all duration-500  ${isMenuOpen ? `translate-x-0` : `translate-x-[100%]`}`}>
                        <ul className="nav-links flex flex-col gap-7 mt-3 p-8">
                            <li onClick={() => setIsMenuOpen(false)}>
                                <Link className='text-xl nav-link' href="/">Home</Link>
                            </li>
                            <li onClick={() => setIsMenuOpen(false)}>
                                <Link className='text-xl nav-link' href="/about">About</Link>
                            </li>
                            <li onClick={() => setIsMenuOpen(false)}>
                                <Link className='text-xl nav-link' href="/contact">Contact</Link>
                            </li>
                            <li onClick={() => setIsMenuOpen(false)}>
                                <Link className='text-xl nav-link' href="/policy">Policy</Link>
                            </li>
                        </ul>
